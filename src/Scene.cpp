...
SceneRenderer::SceneRenderer(int w,int h)
    : width_(std::max(1,w)),height_(std::max(1,h)),
      ...
      cube_(Mesh::cube()),plane_(Mesh::plane(1)),waterPlane_(Mesh::roundedWaterSurface(72,24,0.490f)),
      sphere_(Mesh::uvSphere(18,12)),cylinder_(Mesh::cylinder(18,true)),cone_(Mesh::cone(16)),
      trapezoid_(Mesh::trapezoidPrism()),leaf_(Mesh::leafBlade(9)),disc_(Mesh::disc(24)),screenQuad_(Mesh::plane(1)),
      pondBank_(Mesh::roundedPondBank(72)),rock_(Mesh::irregularRock(14,8)),
      grassPatch_(Mesh::grassPatch(700,1337u)),ricePatch_(Mesh::grassPatch(520,7331u)),
#else
...
void SceneRenderer::resize(int w,int h){
    ...
    width_=w; height_=h;
    const int targetW=std::max(1,(int)std::round((float)w*renderScale_));
    const int targetH=std::max(1,(int)std::round((float)h*renderScale_));
    if(targetW!=fboW_||targetH!=fboH_) recreateSceneFBOs();
}
...
void SceneRenderer::recreateSceneFBOs(){
    destroyFBOs();
    fboW_=std::max(1,(int)std::round((float)width_*renderScale_));
    fboH_=std::max(1,(int)std::round((float)height_*renderScale_));
#ifdef __EMSCRIPTEN__
    ...
    const int bw=1, bh=1;
#else
    const int bw=std::max(1,fboW_/2), bh=std::max(1,fboH_/2);
#endif
...
#ifdef __EMSCRIPTEN__
        // WebGL compatibility: some browser/GPU combinations interpret the depth-only
        // directional shadow texture as a large false shadow over the main 70x62 ground
        // plane.  Keep browser lighting uniform and use the post depth-contact AO for
        // grounding instead.  Native/MSYS2 retains the full realtime shadow path.
        pbr_.set("uReceiveShadow",0);
#else
...
void SceneRenderer::drawPalms(bool shadowPass){
    ...
    for(size_t i=0;i<trees_.size();++i){
...
#ifdef __EMSCRIPTEN__
        const int seg=5, rootCount=2, frondCount=8, oldFrondCount=1;
#else
        const int seg=9, rootCount=4, frondCount=15, oldFrondCount=2;
#endif
        for(int k=0;k<seg;++k){
            float t=(k+0.5f)/seg;
        ...
        for(int k=0;k<rootCount;++k){
            float a=(float)k*2.0f*PI/(float)rootCount+(float)i*0.31f;
        ...
        for(int k=0;k<frondCount;++k){
            float a=(float)k/(float)frondCount*2.0f*PI+(float)i*0.37f;
...
#ifdef __EMSCRIPTEN__
        const int treeBeltCount=24;
#else
        const int treeBeltCount=46;
#endif
        for(int i=0;i<treeBeltCount;++i){
            float a=(float)i/(float)treeBeltCount*2.0f*PI+0.045f*std::sin((float)i*2.3f);
...
#ifdef __EMSCRIPTEN__
        const int birdCount=12;
#else
        const int birdCount=18;
#endif
...
#ifdef __EMSCRIPTEN__
    // WebGL uses an LDR target, so bloom would add two unused FBOs and several full-screen passes.
...
void SceneRenderer::render(const Simulation& sim,const CameraRig& camera){
    ...
    // 1) Directional shadow pass. WebGL omits costly underwater/environment proxies.
    glBindFramebuffer(GL_FRAMEBUFFER,shadowFBO_); glViewport(0,0,shadowSize_,shadowSize_); glClear(GL_DEPTH_BUFFER_BIT);
    glCullFace(GL_FRONT);
#ifdef __EMSCRIPTEN__
    shadow_.use(); shadow_.set("uLightSpace",g.lightSpace); shadow_.set("uTime",sim.time());
    for(int i=0;i<3;++i) drawPondStructure(sim.ponds()[i],i,true);
    drawSensors(sim,true); drawFans(sim,true);
#else
