...
#ifdef __EMSCRIPTEN__
    // Match the DOM/canvas fallback to the renderer so no dark browser-only strip can
    // show around the WebGL surface in day mode on GitHub Pages.
    EM_ASM({
        if (window.AquaNexusSetEnvironment) {
            window.AquaNexusSetEnvironment(!!$0);
        }
    }, night ? 1 : 0);
#endif
}
...
void App::drawUI(){
    ...
    const glm::vec4 panel{0.008f,0.020f,0.038f,0.72f};
    const glm::vec4 panelEdge{0.040f,0.185f,0.245f,0.82f};
    ...
    const glm::vec4 green{0.22f,0.92f,0.52f,1.0f};
    const glm::vec4 cyan{0.16f,0.70f,0.92f,1.0f};
    const glm::vec4 amber{0.94f,0.66f,0.18f,1.0f};
    const glm::vec4 red{0.92f,0.26f,0.30f,1.0f};
    const HudLayout hud=makeHudLayout();
    ...
    for(int i=0;i<3;++i){
        ...
        ui_->roundedRect(badge.x,badge.y,badge.w,badge.h,6,{0.026f,0.086f,0.122f,0.74f});
    ...
    if(width_>760){
        ...
        if(!sensorHudExpanded_){
            ...
            for(size_t i=0;i<rows.size();++i){
                ...
                ui_->roundedRect(row.x,row.y,row.w,row.h,7,{0.014f,0.050f,0.070f,0.72f});
            ...
            ui_->roundedRect(rx+14,qy+21,rw-28,12,6,{0.012f,0.042f,0.060f,0.76f});
            ...
            for(size_t i=0;i<rows.size();++i){
                ...
                ui_->roundedRect(row.x,row.y,row.w,row.h,9,{0.014f,0.050f,0.070f,0.74f});
            ...
            ui_->roundedRect(rx+22,qy+30,rw-44,18,9,{0.012f,0.042f,0.060f,0.80f});
