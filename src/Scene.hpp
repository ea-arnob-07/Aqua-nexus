...
private:
    ...
    // WebGL profile: fewer pixels and a smaller shadow texture reduce browser GPU stalls.
    const int shadowSize_=1536;
    static constexpr float renderScale_=0.72f;
#else
    const int shadowSize_=4096;
    static constexpr float renderScale_=1.0f;
#endif
