...
Shader::Shader(Shader&& o) noexcept
    : program_(o.program_), uniformLocations_(std::move(o.uniformLocations_)) {
    o.program_ = 0;
}
...
GLint Shader::loc(const char* n) const {
    const auto found = uniformLocations_.find(n);
    if (found != uniformLocations_.end()) return found->second;
    const GLint location = glGetUniformLocation(program_, n);
    uniformLocations_.emplace(n, location);
    return location;
}
