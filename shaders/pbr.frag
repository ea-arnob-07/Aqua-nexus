...
float fbm2(vec2 p){
    ...
    for(int i=0;i<FBM_OCTAVES;i++){ s+=a*valueNoise2D(p); p=r*p*2.03+19.17; a*=0.49; }
    return s;
}
...
float shadowPCF(vec4 sp,vec3 N,vec3 L){
    ...
    float s=0.0,w=0.0;
    for(int x=-SHADOW_RADIUS;x<=SHADOW_RADIUS;x++) for(int y=-SHADOW_RADIUS;y<=SHADOW_RADIUS;y++){
