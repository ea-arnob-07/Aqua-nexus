...
#ifdef GL_ES
const int SKY_FBM_OCTAVES=3;
const int SKY_RIDGE_OCTAVES=3;
#else
const int SKY_FBM_OCTAVES=6;
const int SKY_RIDGE_OCTAVES=5;
#endif
...
float fb(vec2 p){ float s=0.0,a=0.52; mat2 r=mat2(.8,-.6,.6,.8); for(int i=0;i<SKY_FBM_OCTAVES;i++){s+=a*n2(p);p=r*p*2.03+11.7;a*=0.49;}return s; }
float ridged(vec2 p){
    float s=0.0,a=0.54;
    for(int i=0;i<SKY_RIDGE_OCTAVES;i++){
